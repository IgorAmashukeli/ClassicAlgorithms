...
void TestComparatorConstructor() {
    MapBST<int, int, std::greater<int>> map_avl{std::greater<int>()};
    assert(map_avl.Empty());
    assert(map_avl.Size() == 0);
...
void TestMoveConstructor() {
    ...
    MapBST<int, int> moved(std::move(original));
...
void TestSwap() {
    ...
    MapBST<int, int> map_a;
    MapBST<int, int> map_b;
    for (int val : input_a) {
...
void TestInsertRValue() {
    MapBST<std::string, std::string> map_avl;
...
void TestWithComplexKeys() {
    ...
    MapBST<ComplexKey, std::string> map_avl;
